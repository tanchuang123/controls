...
{
    setMouseTracking(true);//Qt跟踪鼠标事件：setMouseTracking(true),该属性用来设置某个控件是否被跟踪轨迹。
    setWindowFlags(Qt::FramelessWindowHint);//去掉qt窗体标题栏
...
void NBaseSwitchButton::mousePressEvent(QMouseEvent *event)
{
    if (event->button() == Qt::LeftButton) {
        mMouseDown = true;
        mMouseUp = false;
        firstPoint = event->pos();//鼠标第一次出现位置
    }
}
...
void NBaseSwitchButton::mouseMoveEvent(QMouseEvent *event)
{
    ...
    {
        mMouseMove = true;
        lastPoint = event->pos(); //获取鼠标最后出现的位置
        repaint();
    }
}
